...
simulacao_amostral_empirica <- function(n_sim = 200, 
  ...
  # Gráfico de Poder Empírico com Suavização LOESS
  ggplot(resultados, aes(x = n, y = Poder, color = Teste)) + 
    # Mantemos os pontos originais, mas levemente transparentes para mostrar a "realidade" dos dados empíricos
   # geom_point(size = 3, alpha = 0.4) + 
    # Substituímos o geom_line() pelo geom_smooth() usando o método LOESS
    geom_smooth(method = "loess", se = FALSE, linewidth = 1.2) + 
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.1)) + 
    labs(title = "Poder Empírico: Log-Rank vs Wilcoxon", 
         subtitle = paste("Monte Carlo (", n_sim, " repetições) - Tendência Suavizada via LOESS"), 
         x = "Tamanho Amostral (n)", 
         y = "Poder (Taxa de Rejeição)") + 
    theme_minimal()
}
